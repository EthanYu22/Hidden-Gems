...
Map.prototype = {
  ...
  init: function () {
    ...
    google.maps.event.addListener(this.google_map, 'click', function (event) {
      ...
      google.maps.event.trigger(that.pin_drop.google_marker, 'click');
    });
  },
  ...
  markPlaces: function () {
    ...
    postsRef.once('value', function (snapshot) {
      snapshot.forEach(function (childSnapshot) {
        ...
        var location = new Location(data.id, data.maker_id, data.coord, data.description, data.title, data.rating);
        // Draw the marker
        location.drawMarker();
      })
    });
...
document.getElementById('search-btn').addEventListener('click', function () {
  ...
  google.maps.event.trigger(map.pin_drop.google_marker, 'click');
})